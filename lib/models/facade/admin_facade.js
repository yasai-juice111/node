...
Adminfacade.prototype.index = function(req, params, callback) {
    ...
    async.series({
    	...
    	userLunchBoxList: function(callback) {
    		userLunchBoxDao.getByLunchBoxIdList(req, lunchBoxIdList, function(error, result) {
                if (error) {
                    callback(error);
                    return;
                }
                userIdList = _.uniq(_.pluck(result, 'userId'));
                callback(null, result);
            });
    	},
        userList: function(callback) {
            userDao.getByIdList(req, userIdList, callback);
        }
        ...
        // 整形
        var userLunchBoxList = prepare.userLunchBoxList;
    	_.each(userLunchBoxList, function(userLunchBox) {
            // ランチ情報
            userLunchBox.lunchBox = _.findWhere(prepare.lunchBoxList, { id: userLunchBox.lunchBoxId}) || null;
            // ユーザ情報
            var user = _.findWhere(prepare.userList, { id: userLunchBox.userId}) || null;
            if (user) {
                userLunchBox.employeeId = user.employeeId;
                userLunchBox.employeeType = user.employeeType;
                userLunchBox.company = user.company;
            }
    	});
    	callback(null, {
    		userLunchBoxList : userLunchBoxList
    	});
    });
}
